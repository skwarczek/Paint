      ...
      TMouseButton Button, TShiftState Shift, int X, int Y)
{
  ...
  if (t==linia){   //Narzêdzie rysowania Lini przypisanie wspó³rzêdnych X i Y
   oldt=t;
   t=liniastart;
   startx=endx=X;
   starty=endy=Y;
  ...
  if (t==kwadrat){    //Narzêdzie rysowania Kwadratu przypisanie wspó³rzêdnych X i Y
   oldt=t;
   t=kwadratstart;
   startx=endx=X;
   starty=endy=Y;
  ...
  if (t==kolo){   //Narzêdzie rysowania Ko³a przypisanie wspó³rzêdnych X i Y
   oldt=t;
   t=kolostart;
   startx=endx=X;
   starty=endy=Y;
  ...
  if (t==wypelnienie){     //Narzêdzie wype³nienia obiektów przypisanie wspó³rzêdnych X i Y
        Image1->Canvas->Brush->Color=CColorGrid1->ForegroundColor;
        Image1->Canvas->FloodFill(X, Y, Image1->Canvas->Pixels[X][Y],fsSurface);
}

  if (t==olowek){  //Narzêdzie rysowania O³ówek przypisanie wspó³rzêdnych X i Y
     oldt=t;
     t=olowekstart;
  ...
  if (t==gumka){   //Narzêdzie gumka przypisanie wspó³rzêdnych X i Y
      oldt=t;
      t=gumkastart;
      Image1->Canvas->MoveTo(X,Y);
}      
...
{
t=olowek;  //Przypisanie do przycisku funkcji o³ówka
